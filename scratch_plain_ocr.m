...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        if (!doc) {
            fprintf(stderr, "Failed to load PDF\n");
            return 1;
        }
        
        NSUInteger startPage = 95;
        NSUInteger endPage = 108;
        if (argc >= 3) {
        ...
        for (NSUInteger i = startPage - 1; i < endPage && i < [doc pageCount]; i++) {
            ...
            if (error) {
                fprintf(stderr, "OCR Error on page %lu: %s\n", (unsigned long)(i+1), [[error localizedDescription] UTF8String]);
                continue;
            }
